...
def default_handles() -> dict[str, ModelHandle]:
    ...
    return {
        # Bypassed in the deployed rail: the broker owns model residency now, so model work
        # goes through broker_media and no workflow sets required_model. Kept in step with the
        # live default; a revived direct-to-Ollama path would have to resolve the @role first,
        # since Ollama understands neither a role nor the size-scoped glob this replaced.
        "qwen": OllamaHandle(os.getenv("EDU_LLM_MODEL", "@edu")),
