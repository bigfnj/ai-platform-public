...
def voices_dir() -> Path:
    ...
    # The reference clips live INSIDE this package (packages/edu-media-core/voices), so the
    # package is self-contained and can be relocated without a second thing to remember.
    # This used to be parents[4]/shared/voices, which resolved to rails/edu-suite only because
    # the package happened to sit four levels down inside that rail. The same expression after
    # the move points at <repo-root>/shared/voices, a directory that does not exist — and the
    # broker path always sets VOICES_DIR, so nothing would have failed until someone ran the
    # CLI or a sub-app locally.
