...
- **`tts`** — Coqui XTTS v2 voice-clone engine: one in-memory model, the
  `weights_only` monkey-patch, reference-clip resolution (this package's own
  `voices/`, or `$VOICES_DIR`), `synthesize_segment(text, lang)`, `generate_silence`,
...
- Reference voice clips live INSIDE this package, at `packages/edu-media-core/voices/`,
  and they ARE committed. Keeping them here is what lets the package be relocated
  without a second thing to remember, and `voices_dir()` resolves them relative to
  itself rather than by counting `../` up to a repo root. Override with `$VOICES_DIR`.
  See that folder's README. Never put a student's voice in there; git history is
  permanent.
