...
function currentApp(): string {
  // Empty, not a rail id. The shell must not name a specific rail: `activeEntry` already
  // falls back to railApps[0], so the landing tile is "whatever this user's first entitled
  // rail is" rather than a hardcoded default that goes stale the moment that rail is removed.
...
export default function App() {
  ...
  return (
    ...
    <AppShell
      ...
      topbarExtra={
        <>
          {/* Dictation lives in the chrome, not in a rail: every federated remote renders
              into this same document, so one mic here can type into whichever rail field
              has focus. Read-aloud is the other half and is per-rail (<SpeakButton>), since
              only the rail knows which block of text it means. */}
          <VoiceControls />
          <ModelWidget
            status={status}
            busy={busy}
