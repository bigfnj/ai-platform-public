...
SCHEDULED_TASKS: list[dict[str, Any]] = [
    ...
    {
        "rail": "platform", "task_id": "model-scan", "label": "Model pool scan",
        "icon": "🧠", "method": "LOCAL", "path": "-",
        "description": "Re-scan the broker for installed models and report the inventory (count + "
                       "class split) in this task's last-run status. The Models tab reads the pool "
                       "live, so a model pulled outside the UI shows up on its own — this is the "
                       "hands-off heartbeat that also surfaces the count without opening the tab.",
        "default": {"freq": "daily", "interval": 1, "at": "04:00", "tz": "America/Los_Angeles"},
    },
]
