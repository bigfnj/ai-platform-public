...
export default function RagDemo({ nim, gen }: { nim: NimInfo; gen: GenInfo | null }) {
  ...
  const loadCorpora = () =>
    getJSON<{ corpora: Corpus[]; default?: string | null }>("/api/rag/corpora").then((d) => {
      setCorpora(d.corpora);
      // Pick the corpus the backend NAMES as the default. The sample questions below are
      // written for one specific corpus, and taking corpora[0] made that a side effect of
      // ingest order — a second seed silently moved the demo onto the wrong one.
      const preferred = d.corpora.find((c) => c.slug === d.default) ?? d.corpora[0];
      setCorpusId((prev) => prev ?? preferred?.id ?? null);
    });
      ...
      <div className="rag-layout">
        ...
        <div className="rag-main">
          <div className="askbar">
            ...
            <DictateButton
              small
              onText={(t) => setQuestion((v) => (v ? v + " " + t : t))}
            />
          ...
          <div className="answer card">
            {answer ? (
              <>
                ...
                {/* Read the finished answer aloud (Kokoro via the broker's non-evicting
                    tts_light; browser fallback). Only once the stream is done. */}
                {!streaming && (
                  <div className="ans-acts" style={{ marginTop: 10 }}>
                    <SpeakButton small text={answer} />
                  </div>
                )}
              </>
            ) : (
