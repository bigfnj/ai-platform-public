...
def list_corpora(con: sqlite3.Connection, owner: str | None) -> list[dict]:
    """Seed corpora (shared) + this owner's uploads. Seeds first, in the order they were
    seeded (id ASC); then the owner's uploads, newest first.

    The RAG demo defaults its corpus to the FIRST entry here, so seed order decides the
    default a user lands on. It must be stable and intentional — ``id DESC`` made "whichever
    seed was ingested most recently" the default, so adding a second seed corpus (embedding
    concepts, ingested after nvidia-stack) silently moved the default off the corpus the demo's
    sample questions are written for, and every sample answered "context does not cover".
    Oldest-seed-first keeps the original companion corpus the default as more are added.
    """
    rows = con.execute(
        ...
        "ORDER BY (c.kind='seed') DESC, "
        "         CASE WHEN c.kind='seed' THEN c.id END ASC, "  # seeds: oldest first (stable default)
        "         c.id DESC",                                    # uploads: newest first
        (owner,)).fetchall()
