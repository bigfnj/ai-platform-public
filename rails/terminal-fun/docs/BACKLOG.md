...
## "Web toy" tile type + Buzzed Eyes — BUILT 2026-08-14

A new tile kind for **graphical, non-terminal** toys: a self-contained HTML/canvas page served as a
static file and rendered in a sandboxed `<iframe>` (not xterm/PTY). First one is **Buzzed Eyes 👀**
(a bloodshot cursor-follower trailing cannabis leaves/smoke/glitter/sparkles), ported in from a
standalone side project.

- `Item.web` flag + a sixth **"Web Toys"** shelf (`catalog.py`); assets baked at
  `deploy/rootfs/opt/fun/webtoys/<id>/`, mounted at `/api/webtoys/` (`main.py`, `settings.webtoys_dir`).
- Frontend renders web tiles in an iframe with `sandbox="allow-scripts allow-same-origin"`.
  **`allow-same-origin` is required** — the toy loads its own gateway-gated PNG and an opaque-origin
  frame withholds the session cookie (→ 401 → broken image). First-party trusted content, so it's fine.
- **Follow-up (nice-to-have):** the web-toy pipeline has no automated test (the PTY games have none
  either); a tiny check that `/api/webtoys/eyeballs/index.html` returns 200 would guard the mount +
  asset-baking. Also: future web toys should keep their assets same-origin and stay first-party.
