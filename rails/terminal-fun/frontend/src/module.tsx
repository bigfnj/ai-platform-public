...
import { RailHeader, ModelChips, DictateButton, type ModelChip } from '@web-core'

interface Category { id: string; label: string }
interface Item { id: string; label: string; icon: string; category: string; watch: boolean; info: string; tunable: boolean; saveable: boolean; web: boolean }
...
.ft .chip.web { color:var(--ac); border-color:var(--bd); }
...
.ft .webframe { flex:1; min-height:0; width:100%; display:block; background:var(--s1);
  ...
  return (
    <div className="ft-modal-ov" onClick={onClose}>
      <div className="ft-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="kind">{item.web ? 'Web toy — move your mouse' : item.watch ? 'Watch — no input needed' : 'Play — interactive'}{item.tunable ? ' · tunable by the assistant' : ''}{item.saveable ? ' · saves your progress' : ''}</div>
...
export default function TerminalFunModule() {
  ...
  const [iframeKey, setIframeKey] = useState(0)  // bump to reload a web toy's iframe
  const [caps, setCaps] = useState<Caps | null>(null)  // header chip: assistant model state
  ...
  // Header chip: poll capabilities so the four-state dot tracks the assistant model's residency
  // (and the resolved model tracks the admin Rails panel) without a reload. Same 6s cadence as
  // the other rails. Keeps last-known state on a failed poll rather than blanking the header.
  useEffect(() => {
    let live = true
    const load = () =>
      fetch('/terminal-fun/api/capabilities')
        .then((r) => r.json())
        .then((c: Caps) => { if (live) setCaps(c) })
        .catch(() => {})
    load()
  ...
  useEffect(() => {
    // Web toys render as a static iframe — no PTY/WebSocket to open.
    if (active && !active.web) connect(active)
    return () => teardown()
  ...
  const menu = (
    <div className="menu">
      <div className="lead">Pick something to run right here in the page — a live terminal game, a toy, or an animated web toy. Tap the <b>ⓘ</b> on any tile for how to play, or ask the assistant below. Everything is self-hosted — nothing leaves this box.</div>
      {cats.map((c) => {
              ...
              {mine.map((i) => (
                <div key={i.id} className="tile" role="button" tabIndex={0} onClick={() => setActive(i)}>
                  ...
                  <span className="chips">
                    {i.web
                      ? <span className="chip web">web toy</span>
  ...
  // Graphical web toys aren't terminal programs: render the static toy in a sandboxed iframe so
  // its full-page cursor/canvas effects stay inside the panel. It's first-party content served
  // same-origin, and it loads its own gateway-gated assets (e.g. the leaf PNG) — so it needs
  // allow-same-origin (opaque origin withholds the session cookie → gated subresources 401).
  // The remaining sandbox still blocks top-navigation, popups, and form submission.
  const webView = active && (
    <div className="termwrap">
      <div className="bar">
        ...
        <span className="title"><span>{active.icon}</span>{active.label}</span>
        <span className="sp" />
        <span className="status"><span className="dot on" />running</span>
        <button className="ghost" onClick={() => setHelp(active)}>ⓘ How to play</button>
        <button className="ghost" onClick={() => setIframeKey((k) => k + 1)}>Restart</button>
      </div>
      <iframe
        key={iframeKey}
        className="webframe"
        title={active.label}
        src={`/terminal-fun/api/webtoys/${active.id}/index.html`}
  ...
  return (
    <>
      <RailHeader
        icon="🕹️"
        title="Terminal Fun"
        subtitle="Self-hosted terminal games and toys, with an in-terminal AI helper — sandboxed, no host access."
        chips={
          <ModelChips
      ...
      <div className="ft">
        <div className="body">
          {active ? (active.web ? webView : terminalView) : menu}
        </div>
      <div className="chat">
        <div className="row">
          <input
          ...
          />
          <DictateButton small onText={(t) => setInput((v) => (v ? v + ' ' + t : t))} />
