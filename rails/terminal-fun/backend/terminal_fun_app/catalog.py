...
Item.web=True           -> a graphical "web toy" tile, NOT a terminal program: it runs no
                           PTY and has no argv. The frontend renders it in a sandboxed
                           <iframe> pointed at /api/webtoys/<id>/index.html instead of an
                           xterm session (see webtoys served by main.py + deploy/rootfs).
"""
...
class Item:
    ...
    idle_timeout: int = 0
    info: str = ""
    web: bool = False  # graphical iframe toy (no PTY); served from /api/webtoys/<id>/
...
ITEMS: list[Item] = [
    ...
    # --- Web toys (graphical iframe, no terminal) ---
    Item("eyeballs", "Buzzed Eyes", "👀", "web", web=True, allow_input=False,
         info="A pair of glowing, bloodshot eyes float around and follow your cursor — trailing "
              "smiling cannabis leaves, drifting smoke, gold-and-silver glitter, little firework "
              "bursts, and a haze of twinkling sparkles.\n\n"
              "This one's not a terminal game — it runs right in the panel. Just move your mouse "
              "around inside it. Nothing to press." + _LEAVE),
]
...
def public_catalog() -> list[dict]:
    ...
    return [
        {"id": i.id, "label": i.label, "icon": i.icon, "category": i.category,
         ...
         "saveable": is_saveable(i.id), "web": i.web}
        for i in ITEMS
    ]
