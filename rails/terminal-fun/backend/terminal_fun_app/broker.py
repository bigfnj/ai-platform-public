"""Thin client to the platform GPU/Model Broker (adapted from the finance rail).
...
def _get(path: str, timeout: float = 30.0) -> dict:
    try:
        resp = httpx.get(_base() + path, timeout=timeout, headers=_AUTH)
...
def models() -> list[dict]:
    """List the models the broker can serve (Ollama tags). Used by the header chips to tell
    whether the assistant role's model is installed. Tolerant of the two response shapes."""
    resp = _get("/v1/models")
...
def roles() -> list[dict]:
    ...
    name the actual model behind a @role (e.g. @terminal-fun -> gemma4:12b)."""
    resp = _get("/v1/roles")
