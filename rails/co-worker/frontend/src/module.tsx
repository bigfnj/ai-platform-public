...
import { DictateButton, RailHeader, ModelChips } from '@web-core'
...
// The shared @web-core <ModelChips>: co-worker's ModelsStatus already carries the ModelChip shape
// per slot (slot/label/model/state/role), so status.models forwards straight through. The
// harvested-item count becomes the trailing note, and the fetch lifecycle maps onto the
// component's three states — null (first poll in flight) → checking, broker down → unreachable,
// otherwise ok. Same four-state dot semantics as before, now from the one shared component.
function CoWorkerChips({ status }: { status: ModelsStatus | null }) {
  return (
    <ModelChips
      status={status ? (status.broker === 'ok' ? 'ok' : 'unreachable') : 'checking'}
      models={status?.models}
      trailing={status ? `${status.items} items harvested` : undefined}
    />
  )
}
  ...
  return (
    <div className="co-worker">
      <RailHeader
        icon="💼"
        title="Co-Worker"
        subtitle="Email, calendar & Teams intelligence — harvested, ranked, client-first"
        chips={<CoWorkerChips status={models} />}
        actions={
          <>
      ...
      {err && (
        <div className="cw-err">
          ...
          <br />
          Is the <code>co-worker</code> service up on :8890?
        </div>
      )}
      ...
      {items.length > 0 && (
        <div className="cw-bar">
          ...
          <DictateButton
            onText={(text) => setQ((prev) => (prev ? `${prev} ${text}` : text))}
          />
