...
from co_worker_app.identity import identity
...
# Identity is required app-wide (bouquet's precedent). Nothing here was gated. Eight routes
# declared `x_platform_user: str = Header(default="?")` and not one of them ever read the
# value — it looked like authentication and was only decoration, so a request with no gateway
# header sailed through with the caller recorded as nothing at all. That mattered more here
# than on most rails: this inbox is a real person's harvested email, calendar and Teams
# traffic, /api/doc will hand back any markdown under the inbox, PATCH /api/inbox/{id} rewrites
# triage state, and POST /api/brief/refresh spends a model run on demand. An app-level
# dependency is used rather than one per route so a new route cannot be added un-gated by
# omission — which is exactly how the placeholder parameters spread in the first place.
#
...
def inbox_get(item_id: str) -> dict[str, Any]:
...
def inbox_patch(item_id: str, patch: StatusPatch) -> dict[str, Any]:
...
def doc_get(doc_path: str) -> dict[str, Any]:
