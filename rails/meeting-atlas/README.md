...
- Ports: backend **8740**, vite dev **5340**
...
| `POST /api/reindex` | rebuild from disk — the hook an ingest task calls; **admin-only** |

Every route requires a platform identity (`X-Platform-User`, set by the gateway); a request
without one is a 401, not an anonymous read. `/docs` and `/openapi.json` are off, because doc
routes bypass the app-level dependency. Set `PLATFORM_STANDALONE=1` to run the backend with no
gateway in front, for local dev.
...
`MEETING_ATLAS_MEETILY_DB_MOUNT`. Both fall back to empty named volumes, so a missing mount
yields an empty rail rather than a broken stack.

⚠ `MEETING_ATLAS_MEETILY_DB_MOUNT` names the database **file**, not the directory holding it, and
`MEETING_ATLAS_MEETILY_DB` above is derived from it (`:+` in compose) rather than set separately.
There used to be a second variable, `MEETING_ATLAS_MEETILY_DB_IN_CONTAINER`, and it is gone: two
operator variables for one path meant setting either alone gave a mounted-but-unread database or a
configured-but-missing one, with no symptom except titles quietly falling back to the folder name.
An `.env` carrying the old directory form will bind a directory onto a file path.
...
cd frontend && npm install && npm run dev   # http://localhost:5340
```
