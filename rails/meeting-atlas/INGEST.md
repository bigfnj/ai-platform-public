...
This hook is **admin-only**: it rebuilds the whole index and swaps it out from under every
reader, so the caller has to be a platform admin (the gateway forwards `X-Platform-Admin: 1`,
and a scheduled platform job counts as one). A signed-in non-admin gets a 403; a caller with
no platform session at all gets a 401, as it does on every other route here.
