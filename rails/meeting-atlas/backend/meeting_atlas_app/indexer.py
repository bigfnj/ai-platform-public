...
def read_db(db_path, log=None):
    ...
    if not db_path or not os.path.isfile(db_path):
        return out
    # A PRIVATE directory per call, removed afterwards. This used to be one fixed filename in
    # the shared temp dir, and -wal/-shm were copied only when the SOURCE had them — so a
    # previous read's leftover -wal was replayed over the freshly copied database and this
    # read returned the PREVIOUS database's rows. The fixed name was also a collision between
    # concurrent builds, and between this rail and anything else on the box.
    tmpdir = tempfile.mkdtemp(prefix="ma-meetily-")
    tmp = os.path.join(tmpdir, "snapshot.sqlite")
    con = None
    try:
...
# A due that is explicitly "no date yet". Shared by _mark_shared and
# _due_implausible so the two cannot drift into disagreeing about what a date is.
_PLACEHOLDER_DUE = re.compile(r"\b(tbd|asap|ongoing|n/?a|none|immediate)\b")
...
def _mark_shared(items):
    ...
    """
    # Only dates that could BE a date. _due_implausible deliberately whitelists TBD/ASAP/etc,
    # so counting them here flagged a summary for following the instructions: three items
    # correctly marked TBD (which INGEST.md tells writers to use) all got due_uniform.
    dues = Counter(i["due"] for i in items
                   if i.get("due") and not _PLACEHOLDER_DUE.search(i["due"].lower()))
...
# Delimiter PAIRS, so the closing mark is the only thing excluded from the body. The old
# single character class lumped the straight apostrophe in with the closers, which truncated
# every quote at its first contraction: `"the team's runbook is not written"` extracted as
# `the team`, matched some unrelated segment, and came back with a quote_at — i.e. a
# FABRICATED citation was reported as verified, which is the one thing this rail exists to
# catch. A straight `'` is no longer accepted as a delimiter at all: it is ambiguous with an
# apostrophe, and most spoken English contains contractions.
_QUOTE_RE = re.compile(u'"([^"]{6,})"' u"|“([^”]{6,})”" u"|‘([^’]{6,})’")


def _extract_quote(ref):
    if not ref:
        return None
    m = _QUOTE_RE.search(ref)
    candidate = next((g for g in m.groups() if g), None).strip() if m else None
    if candidate is None:
        candidate = TS_BRACKET.sub("", ref).strip(" *_|")
    # One threshold, measured the way find_quote measures it. Extraction used to accept 6 raw
    # characters while find_quote demanded 8 NORMALISED, so a genuine short quote was extracted,
    # failed to match, and got reported as fabricated. Erring toward not checking a very short
    # quote is a false negative; erring the other way accuses a correct summary.
    return candidate if len(norm(candidate)) >= 8 else None
...
def build_index(recordings_dir, db_path=None, tz=None, log=None):
    ...
    for m in loaded:
        ...
        if m["sidecar"]:
            # Provenance fields are replaced WHOLESALE, not merged. Dropping None let a
            # summary.json with no "model" key inherit the Meetily database's summary_model,
            # so a co-work summary was labelled with the local model while reporting
            # summary_source: "sidecar" — exactly what INGEST.md promises cannot happen
            # ("a summary is never silently attributed to the wrong model"). If the sidecar
            # owns the summary it owns its attribution, including saying nothing.
            side = m["sidecar"]
            rec.update({k: v for k, v in side.items()
                        if v is not None or k in ("summary_model", "summary_secs")})
