...
payload has this bug — it reports ``installed: false`` for bge-m3 on a box where bge-m3 is
installed and resident. That is why this module does its own comparison rather than trusting
the ``installed`` flag, and why ``_same()`` must not be "simplified" to ``==``.

This module is duplicated verbatim (bar the import line) in the co-worker and terminal-fun
rails. That is deliberate: rails are independent deployables with their own images and their
own dependency sets, and there is no shared Python package they all install. Keep the four
state names and the resolution order identical across the copies — the chips are a
cross-rail visual language, and a rail that computes "warming" differently is a lie.
"""
