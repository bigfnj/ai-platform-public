"""Ingest the GECX knowledge base into the chunk index.
...
from gemini_cx import broker, config, rag, store

log = logging.getLogger("gemini_cx.ingest")
...
def ingest_seed(*, force: bool = False) -> dict:
    ...
    for folder in sorted(p for p in root.iterdir() if p.is_dir() and not p.name.startswith("_")):
        ...
        if not rows:
            # A collection that has *become* empty must be cleared, not skipped. Without this,
            # emptying a folder leaves its old chunks indexed forever and they keep being
            # retrieved and cited through every later edit, because nothing ever removes them.
            # This bit the sibling smb-partner-enablement rail for weeks — do not "simplify" it.
    ...
    # A collection whose folder was deleted outright is never iterated above, so its chunks
    # would otherwise survive indefinitely. Retiring a renamed collection hits exactly this.
...
def ingest_upload(name: str, text: str, *, source: str) -> int:
    """Index an ad-hoc document a user uploaded, as its own 'upload' collection."""
