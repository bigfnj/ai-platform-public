...
import { RailHeader, ModelChips, DictateButton } from '@web-core'
...
// Model chips are the shared @web-core <ModelChips>: caps.models is already the ModelChip shape
// (slot/label/model/state/role), so this just forwards it, adds the corpus summary as trailing
// text, and maps the caps lifecycle onto the component's three states (checking → ok/unreachable).
function GeminiChips({ caps }: { caps: Capabilities | null }) {
  return (
    <ModelChips
      status={caps ? (caps.broker === 'ok' ? 'ok' : 'unreachable') : 'checking'}
      models={caps?.models}
      trailing={caps ? `${caps.corpus.chunks} chunks · ${caps.corpus.collections} collections` : undefined}
    />
  )
}
...
export default function GeminiCxModule() {
  ...
  // Cancel any in-flight stream (and its watchdog) if the module unmounts mid-answer.
  useEffect(() => () => {
    cancelRef.current?.()
  ...
  const run = useCallback((req: AskRequest, label: Asked, id: string) => {
    ...
    // Streaming is a progressive enhancement, not a requirement. The WebSocket rides through
    // Cloudflare, which carries the handshake and the first (retrieval) frame but has been seen
    // to stall the small token frames that follow — no tokens, no clean close, so a close-only
    // fallback never fires and the panel hangs at 'Retrieving and reasoning…'. So: if no answer
    // token arrives quickly, OR the stream ends without one, abandon it and use the buffered
    // /api/ask — plain HTTP, the same path that already serves the deck and sources here.
    let gotToken = false
    let settled = false
    ...
    const useBuffered = () => {
      ...
      settled = true
      if (watchdogRef.current) window.clearTimeout(watchdogRef.current)
      cancelRef.current?.()          // close the WebSocket if it is still open
      cancelRef.current = null
      askBuffered(req)
        .then((r) => { setAnswer(r.answer); setCites(r.citations) })
    ...
    const cancel = askStreaming(
      ...
      (err) => {
        cancelRef.current = null
        if (settled) return
        if (!gotToken) { useBuffered(); return }   // stream ended without an answer
        settled = true
        if (watchdogRef.current) window.clearTimeout(watchdogRef.current)
    ...
    // If the stream produces no token within this window (warm first-token is ~4s), assume the
    // transport is swallowing them and switch to buffered. Cleared the instant a token lands.
    watchdogRef.current = window.setTimeout(() => { if (!gotToken) useBuffered() }, 12000)
  }, [])
  ...
  return (
    <div className="gemini-cx">
      <RailHeader
        icon={<GeminiIcon />}
        title="Gemini Enterprise CX"
        subtitle="Grounded answers on Google Cloud's Gemini Enterprise for Customer Experience — every claim cited, status never smoothed over"
        chips={<GeminiChips caps={caps} />}
      />
