...
cd rails/gemini-cx
pip install -e .
PLATFORM_STANDALONE=1 uvicorn --factory gemini_cx.api:create_api --port 8880 \
  --reload --reload-dir src
```

`PLATFORM_STANDALONE=1` is required outside the gateway: identity fails closed by design, so
...
npm install && npm run dev      # http://localhost:5330
```
