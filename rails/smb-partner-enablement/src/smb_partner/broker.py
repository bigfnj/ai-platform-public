...
RAG LLM on the same card, so there is no GPU gate and no model swap per utterance.

There was also a ``tts()`` wrapper for the broker's XTTS path, kept "for completeness" and never
called. It is gone: an uncalled method pointing at a subsystem this rail deliberately avoids is
not completeness, it is a false positive waiting for the next dependency sweep — and it produced
exactly that when the XTTS consumers were audited on 2026-08-26.
"""
