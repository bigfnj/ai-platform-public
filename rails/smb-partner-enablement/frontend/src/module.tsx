...
import { ask, askStream, getCapabilities, transcribe } from './api'
...
// The model-state dots are now the shared @web-core <ModelChips> (the four-state red/blue/
// orange/green contract lives there, on .rail-chip-dot). caps.models is already the ModelChip
// shape (slot/label/model/state/role), so it forwards straight through, and the caps lifecycle
// maps onto the component's three states: null → checking, broker !== 'ok' → unreachable, else
// ok. This rail's own extras — the binary Voice indicator and the corpus summary — ride in the
// `trailing` slot so they still sit after the model chips, as the old inline status row showed.
function SmbChips({ caps }: { caps: Capabilities | null }) {
  return (
    <ModelChips
...
function VoiceChat({ caps }: { caps: Capabilities | null }) {
  ...
  const submit = useCallback(
    (text: string, spoken: boolean) => {
      ...
      // Streaming is a progressive enhancement. The WebSocket rides through Cloudflare, which
      // carries the handshake and the citations frame but has been seen to stall the token
      // frames from a slower (24B) model — no tokens, no clean close — leaving the turn stuck
      // 'streaming' forever. So if no token arrives quickly, OR the stream ends without one,
      // fall back to the buffered /api/ask (plain HTTP, the path that already serves this rail).
      let gotToken = false
      let settled = false
      let watchdog: number | undefined
      let cancelWs: (() => void) | null = null

      const useBuffered = () => {
        if (settled) return
        settled = true
        if (watchdog) window.clearTimeout(watchdog)
        cancelWs?.()   // close the stalled socket if still open
        ask({ question: q, speak: spoken })
          .then((r) => {
            patch((t) => ({ ...t, answer: r.answer, citations: r.citations,
                                  grounded: r.grounded, streaming: false }))
            if (spoken && r.voice) { setSpeaking(true); speak(r.voice, () => setSpeaking(false)) }
          })
          .catch((e: unknown) => patch((t) => ({ ...t, streaming: false,
      ...
      cancelWs = askStream(
        { question: q, speak: spoken },
        {
          ...
          onToken: (tok) => {
            if (!gotToken) { gotToken = true; if (watchdog) window.clearTimeout(watchdog) }
            patch((t) => ({ ...t, answer: t.answer + tok }))
          },
          onDone: ({ answer, voice }) => {
            if (settled) return
            settled = true
          ...
          onError: (detail) => {
            if (settled) return
            if (!gotToken) { useBuffered(); return }   // failed before any answer
            settled = true
            if (watchdog) window.clearTimeout(watchdog)
      ...
      // No token within the window (warm first-token is a few seconds) ⇒ the transport is
      // swallowing them; switch to buffered. Cleared the instant a token lands.
      watchdog = window.setTimeout(() => { if (!gotToken) useBuffered() }, 12000)
    },
    [busy],
  )
...
export default function SmbPartnerModule() {
  ...
  return (
    <div className="smbp">
      {/* The shared @web-core <RailHeader>: icon · title · subtitle · chips, with the rail's own
          audio controls in the actions slot. The 🤝 is this rail's catalog.py icon, so the
          header matches the sidebar item that opened it. The tab bar below stays put — it is a
          separate row, not part of the header. */}
      <RailHeader
        icon="🤝"
        title="SMB Partner Enablement"
        subtitle="A two-minute meeting kit for Microsoft SMB partners — grounded only in curated SME material, every claim cited"
        chips={<SmbChips caps={caps} />}
        actions={<AudioDevicePicker />}
      />
