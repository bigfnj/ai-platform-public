...
pip install -e .
PLATFORM_STANDALONE=1 uvicorn --factory smb_partner.api:create_api --port 8870

# desktop remote            # mobile surface
npm run dev                 npm run dev:mobile
```

`PLATFORM_STANDALONE=1` is required outside the platform: identity fails closed on a missing
