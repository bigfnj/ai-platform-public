...
def test_ungated_caller_is_refused(client):
    # Inverted deliberately. This used to assert that a header-less caller PASSES require_admin,
    # which is what the gate actually did: it only rejected a named non-admin, so a sibling
    # container with no header could kick off GPU icon generation. A request with no identity
    # did not come through the gateway, so it is now a 401.
    assert client.post("/api/icons/generate").status_code == 401


def test_standalone_dev_may_still_trigger(client, monkeypatch):
    # The dev convenience the old test was really protecting, now behind an explicit flag.
