...
def test_ungated_upload_is_refused(client):
    # Inverted deliberately: a header-less caller used to author recipes into any category it
    # chose, bypassing the To-Try bucket that constrains real non-admin contributors.
    assert client.post("/api/recipes", json=_COMMIT).status_code == 401


def test_standalone_upload_keeps_category(client, monkeypatch):
    # standalone/dev is not a contributor -> keeps the chosen category
