...
## DONE 2026-09-01 — adding or editing a recipe now updates the semantic index

Contributing a recipe left it out of semantic search, and editing a title or ingredients left a
stale vector, because only a full admin/scheduler re-embed of the whole catalog ever wrote the
index. `semantic.add()` handles one recipe per embed call (replacing in place) and all three
routes queue it. Platform `docs/BACKLOG.md` has the reasoning; `tests/test_semantic_add.py` is
the guard, mutation-tested per route.

Follow-on, not done: there is **no delete-recipe route**, so nothing orphans a vector today. If
one is ever added it must drop the id from the index, or search will keep returning a card that
no longer exists.
