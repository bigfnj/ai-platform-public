...
from recipe_book import broker, config, db, icons, ingest, maintenance, modelstate, seed, semantic, state
...
def create_api() -> FastAPI:
    ...
    def capabilities(_: deps.Identity = Depends(deps.identity)) -> dict:
        ...
        icon backend (@recipe-icon) is deliberately excluded: it's an image/media pipeline, not an
        Ollama tag, so the four-state resolver would always report it 'missing'."""
        return modelstate.resolve([
            ("assistant", "Assistant", broker.ASSISTANT_MODEL),
            ("vision", "Vision", broker.VISION_MODEL),
        ])
