...
from recipe_book import authoring, bar, broker, db, extraction, icons, ingest, semantic, state
...
def create_recipe(req: CommitReq, background: BackgroundTasks,
    ...
    # Non-admins can only contribute to the To-Try bucket; admins choose freely. `user is None`
    # now means a standalone dev run (identity() 401s otherwise), not "no header", so this is
    # a dev convenience rather than a hole a sibling container can walk through.
    ...
    try:
        ...
        con.close()
    state.reload()
    background.add_task(_index_recipe, rid)
...
def edit_title(recipe_id: str, req: TitleReq, background: BackgroundTasks,
    ...
    try:
        ...
        con.close()
    state.reload()
    background.add_task(_index_recipe, recipe_id)   # the title is part of the embedded text
...
def edit_content(recipe_id: str, req: ContentReq, background: BackgroundTasks,
                 _admin: deps.Identity = Depends(deps.require_admin)) -> dict:
    ...
    try:
        ...
        con.close()
    state.reload()
    background.add_task(_index_recipe, recipe_id)   # ingredients are part of the embedded text
...
def _index_recipe(recipe_id: str) -> None:
    """Fold the new card into the semantic index so it is searchable immediately.

    Best-effort on purpose: a broker outage must not fail an upload that already wrote a
    real card to the corpus. A shortfall stays visible through ``/api/search/status`` —
    its count against ``/api/health`` total is exactly what the content sweep asserts, and
    is what caught this being missing in the first place.
    """
    r = state.catalog().get(recipe_id)
    if r is None:
        return
