...
/** Which edges of a scroll container have content hidden past them: '', 'up', 'down', or
 *  'up down'. The app rail hides its scrollbar (see styles.css), so this is what tells the
 *  user there is more of the list; it stays empty whenever the rail fits, which is the
 *  normal case. `deps` re-measures when the item count changes, since adding a rail grows
 *  scrollHeight without resizing the nav itself. */
function useScrollEdges(ref: { current: HTMLElement | null }, deps: number): string {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (hidden <= 1) return setEdges('')
      const up = el.scrollTop > 1
      const down = el.scrollTop < hidden - 1
      setEdges([up ? 'up' : '', down ? 'down' : ''].filter(Boolean).join(' '))
    }
    measure()
...
export function AppShell({
  ...
  const railEdges = useScrollEdges(railRef, apps.length)
  return (
    <div className="pshell">
      <nav className="app-rail" aria-label="Apps" ref={railRef} data-scroll={railEdges}>
        {onBrandClick ? (
          <button
