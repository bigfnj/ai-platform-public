...
Spec (in.json), one of:
  ...
  {"op": "kokoro_tts", "text": "...", "voice": "af_heart", "lang_code": "a",
   "speed": 1.0, "model_path": "...", "voices_path": "..."}
  {"op": "transcribe", "audio_b64": "<b64 recording>", "suffix": ".webm",
   "model": "small.en", "device": "cpu", "compute_type": "int8", "language": null}

Result (out.json):
  image      -> {"images": ["<b64 png>"|null, ...], "errors": [...]}
  tts        -> {"audio_b64": "<b64 wav>", "sample_rate": 24000, "timings": [...]}
  kokoro_tts -> {"audio_b64": "<b64 wav>", "sample_rate": 24000, "voice": "...", "lang": "..."}
  transcribe -> {"text": "...", "language": "en", "duration": 4.18, "model": "small.en"}
...
def do_kokoro(spec: dict) -> dict:
    """Kokoro-82M ONNX text-to-speech. Independent of edu_media_core and of torch — it is
    onnxruntime + soundfile only, which is exactly why it can live in this venv without
    touching the fragile torch+cu130 install the image path depends on.

    At ~350 MB it is a transient spike rather than a permanent tenant, so the broker calls
    this WITHOUT the GPU gate and without evicting anything (see Broker.tts_light). Runs on
    CPU under plain onnxruntime; installing onnxruntime-directml would move it to the GPU,
    which is deliberately not done — read-aloud must never contend with the resident LLM.

    Required: model_path (kokoro-v1.0*.onnx) and voices_path (voices-v1.0.bin), passed in
    from BROKER_KOKORO_MODEL_PATH / BROKER_KOKORO_VOICES_PATH.

    lang_code is Kokoro's single-letter code; the kokoro-onnx `create()` API wants a BCP-47
    tag instead, so the two are mapped here. 'a' = American English (default), 'e' = Spanish.
    """
    import numpy as np
    import soundfile as sf
    from kokoro_onnx import Kokoro

    lang_map = {"a": "en-us", "b": "en-gb", "e": "es", "f": "fr", "j": "ja", "z": "zh",
                "p": "pt", "i": "it", "h": "hi"}

    model_path = str(spec.get("model_path") or "")
    voices_path = str(spec.get("voices_path") or "")
    if not model_path or not voices_path:
        raise RuntimeError("kokoro_tts needs model_path and voices_path "
    ...
    code = str(spec.get("lang_code") or "a")
    ...
    return {"audio_b64": base64.b64encode(buf.getvalue()).decode(),
            "sample_rate": int(sample_rate), "voice": voice, "lang": lang}


def do_transcribe(spec: dict) -> dict:
    """faster-whisper speech-to-text. CPU/int8 by default and torch-free (CTranslate2), so
    like kokoro_tts it runs without the GPU gate and evicts nothing.

    CPU is a deliberate default, not a limitation: speech input is the one call where a user
    is sitting there having just stopped talking, so it must never queue behind the GPU gate
    or displace the model they are about to ask. A short utterance transcribes in ~1-2 s on
    CPU, which is faster than waiting for a model swap would be.

    The browser sends whatever MediaRecorder produced — webm/opus on Chrome, ogg on Firefox,
    mp4 on Safari. PyAV bundles the demuxers, so the bytes are written to a temp file with
    the caller's suffix and handed over as-is; no transcode step.

    Weights come from the HuggingFace cache (Systran/faster-whisper-<model>), so the FIRST
    call on a cold cache downloads (~500 MB for small) and later ones are local.

    The default is the MULTILINGUAL model. An ``.en`` variant is English-only and silently
    ignores ``language``, turning Spanish into nonsense — which matters on a bilingual
    platform.
    """
    import shutil
    import tempfile
    ...
    audio_b64 = spec.get("audio_b64")
    if not audio_b64:
        raise RuntimeError("transcribe requires audio_b64")

    model_name = str(spec.get("model") or "small")
    ...
    tmp = tempfile.mkdtemp(prefix="broker-stt-")
    clip = Path(tmp) / f"clip{suffix}"
    try:
        clip.write_bytes(base64.b64decode(audio_b64))
        model = WhisperModel(model_name, device=device, compute_type=compute_type)
        # beam_size=1 is greedy (fast, and plenty for dictation); vad_filter drops the
        # leading/trailing silence a click-to-record UI always produces.
        ...
        return {"text": text, "language": info.language,
                "duration": round(float(info.duration), 2), "model": model_name}
    finally:
