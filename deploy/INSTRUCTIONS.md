...
- **Blank content / "coming soon" for an app?** The mounted frontend dists may
  ...
  ```powershell
  cd D:\.ai-work\projects\ai-platform\apps\platform\frontend ; npm run build
  cd D:\.ai-work\projects\ai-platform\rails\edu-suite\apps\dashboard\frontend ; npm run build
  cd D:\.ai-work\projects\job-aid\frontend ; npm run build
  ```

- **Job Aid data:** job-aid's DB + résumés live on the `job_aid_data` volume, seeded
  once from `D:\.ai-work\projects\job-aid\var\`. New data stays in the volume; to reseed
  or pull it back, use `docker cp` / a one-shot `docker run -v job_aid_data:/data ...`.
