...
1. **Build both frontends** (mounted into the gateway):
   ```powershell
   cd D:\.ai-work\projects\ai-platform\apps\platform\frontend ; npm ci ; npm run build
   cd D:\.ai-work\projects\ai-platform\rails\edu-suite\apps\dashboard\frontend ; npm ci ; npm run build
   ```
