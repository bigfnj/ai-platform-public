...
try {
    ...
    # The invoking user's profile. Podman keeps its machine config and SSH key there, and the
    # service (LocalSystem) is given this via AppEnvironmentExtra so it can reach the machine.
    #
    # Prefer the real $env:USERPROFILE over deriving it from the install path. The derivation
    # only holds when the repo sits directly in the profile (C:\Users\<user>\ai-platform, where
    # get.ps1 clones it); a checkout anywhere else — this repo lives on a separate drive under
    # D:\...\projects\ai-platform — yields the wrong directory, and the service then starts a
    # podman machine that cannot find its own config.
    $UserProfile = if ($env:USERPROFILE) { $env:USERPROFILE } else { Split-Path $Root -Parent }
